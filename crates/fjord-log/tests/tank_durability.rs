...
fn tank_dir() -> Option<PathBuf> {
    let dir =
        PathBuf::from("/tank/services/fjord-objtest").join(format!("run-{}", std::process::id()));
...
async fn produce_fetch_durable_on_tank_filesystem() {
    ...
    let store1: Arc<dyn BlobStore> = Arc::new(ObjectLogBlobStore::new(Arc::new(
    ...
    let store2: Arc<dyn BlobStore> = Arc::new(ObjectLogBlobStore::new(Arc::new(
        LocalObjectStore::new(&dir),
    )));
