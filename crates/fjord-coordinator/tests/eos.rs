...
fn check_eos(c: &dyn CoordinatorStore) {
    ...
    assert!(matches!(out[0], CommitOutcome::Assigned { base_offset: 0, record_count: 3 }));
    // Open txn holds the LSO at its first offset; data is sequenced (HW=3) but
    // not yet stable.
    assert_eq!(c.high_watermark("t", p).unwrap(), 3);
    assert_eq!(c.last_stable_offset("t", p).unwrap(), 0, "open txn must pin LSO at its first offset");
    c.txn_offset_commit(a.producer_id, "g", "t", p, 100).unwrap();
    // Staged offset is NOT visible until commit.
    assert_eq!(c.offset_fetch("g", "t", p).unwrap(), None, "txn offset must not apply before commit");
    c.end_txn(a.producer_id, true).unwrap();
    assert_eq!(c.last_stable_offset("t", p).unwrap(), 3, "commit releases LSO to HW");
    assert_eq!(c.offset_fetch("g", "t", p).unwrap(), Some(100), "commit applies staged offset");
    ...
    assert_eq!(c.last_stable_offset("t", p).unwrap(), 3, "open txn B pins LSO at offset 3");
    c.txn_offset_commit(b.producer_id, "g", "t", p, 999).unwrap();
    c.end_txn(b.producer_id, false).unwrap();
    assert_eq!(c.last_stable_offset("t", p).unwrap(), 5, "abort releases LSO to HW");
    // Abort discards the staged offset — still the committed value from txn A.
    assert_eq!(c.offset_fetch("g", "t", p).unwrap(), Some(100), "abort must NOT apply staged offset");
    let aborted = c.aborted_transactions("t", p, 0).unwrap();
    assert_eq!(aborted, vec![(b.producer_id, 3)], "aborted range recorded for read_committed");
    lso_history.push(c.last_stable_offset("t", p).unwrap());
    ...
    assert_eq!(c.last_stable_offset("t", p).unwrap(), 5, "open txn C pins LSO at offset 5");
    c.end_txn(cc.producer_id, true).unwrap();
    assert_eq!(c.high_watermark("t", p).unwrap(), 7);
    assert_eq!(c.last_stable_offset("t", p).unwrap(), 7, "all txns ended -> LSO = HW");
    lso_history.push(c.last_stable_offset("t", p).unwrap());
    ...
    let aborted_first: Vec<i64> = c.aborted_transactions("t", p, 0).unwrap().iter().map(|(_, f)| *f).collect();
    assert_eq!(aborted_first, vec![3]);
    let committed: Vec<i64> = (0..lso).filter(|o| !(*o == 3 || *o == 4)).collect();
    assert_eq!(committed, vec![0, 1, 2, 5, 6], "read_committed view must be exactly the committed records");
    ...
    c.txn_offset_commit(d.producer_id, "g", "t", p, 200).unwrap();
    ...
    assert_eq!(c.offset_fetch("g", "t", p).unwrap(), off_after_first, "end_txn retry double-applied");
    assert_eq!(c.last_stable_offset("t", p).unwrap(), lso_after_first, "end_txn retry moved LSO");
    ...
    assert_eq!(e1.producer_id, e2.producer_id, "transactional.id keeps a stable producer id");
    assert!(e2.producer_epoch > e1.producer_epoch, "re-init must bump the epoch");
